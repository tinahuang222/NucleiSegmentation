...
import helper_functions as pickle_helper
import nuclei_feature_extraction as extract


__all__ = ['feature_generation']
...
def feature_generation(he_image_pickle, mask_dir, save_dir, bug_dir, mask_ext = '.csv'):
    ...
    if os.path.exists(mask_dir) == False:
        print("Cannot find directory: ", mask_dir, ". Please check whether directory has been correctly entered.")
        exit(-1)
    ...
    #created bug_dir if needed
    if os.path.exists(bug_dir) == False:
        print("created_directory: ", bug_dir)
        os.mkdir(bug_dir)
    ...
    mask_name = list(map(pickle_helper.only_appropriate_file_name, mask_files))
